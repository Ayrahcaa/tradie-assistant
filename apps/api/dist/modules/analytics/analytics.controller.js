...
export async function businessOverviewHandler(request, response, next) {
    try {
        response.json({ data: await getBusinessOverview(request.authUser.id) });
    }
    catch (error) {
        next(error);
    }
}
export async function projectOverviewHandler(request, response, next) {
    try {
...
export async function customerOverviewHandler(request, response, next) {
    try {
        const id = idSchema.safeParse(request.params.customerId);
        if (!id.success) {
            response.status(400).json({ message: "Invalid customer ID." });
            return;
        }
