...
function projectFinancials(project: {
  quotedValue: unknown;
  invoices: Array<{ status: string; totalAmount: unknown; amountPaid: unknown; balanceDue: unknown }>;
  expenses: Array<{ category: string; status: string; amount: unknown; paidAt: Date | null; receipts?: unknown[] }>;
  subcontractorProjectCosts: Array<{ status: string; agreedAmount: unknown; amountPaid: unknown; amountPending: unknown }>;
  quotes?: Array<{ status: string; totalAmount: unknown }>;
}) {
...
const projectInclude = {
  customer: true,
  quotes: { include: { customer: true }, orderBy: { createdAt: "desc" as const } },
  invoices: { include: { customer: true, payments: { orderBy: { paidAt: "desc" as const } } }, orderBy: { issueDate: "desc" as const } },
  expenses: { include: { receipts: true }, orderBy: { expenseDate: "desc" as const } },
  subcontractorProjectCosts: { include: { subcontractor: true, payments: { orderBy: { paidAt: "desc" as const } } }, orderBy: { createdAt: "desc" as const } },
};

export async function getProjectOverview(ownerId: string, projectId: string) {
...
export async function getCustomerOverview(ownerId: string, customerId: string) {
...
export async function getBusinessOverview(ownerId: string) {
  ...
  const directExpenseFor = (key: string) => sum(directExpenses.filter((expense) => monthKey(expense.expenseDate) === key), (expense) => number(expense.amount));
  const subcontractorOutFor = (key: string) => sum(recentSubcontractorPayments.filter((payment) => monthKey(payment.paidAt) === key), (payment) => number(payment.amount));
  const revenueFor = (key: string) => sum(paymentRevenue.filter((payment) => monthKey(payment.paidAt) === key), (payment) => number(payment.amount));
  ...
  const reportTrend = Array.from({ length: 24 }, (_, index) => {
    const date = new Date(year - 1, index, 1); const key = monthKey(date); const revenue = revenueFor(key); const expensesForMonth = directExpenseFor(key) + subcontractorOutFor(key);
  ...
  const dueSoonCutoff = new Date(now); dueSoonCutoff.setDate(now.getDate() + 7);
  const invoiceBuckets = issuedInvoices.reduce((result, invoice) => {
    const amount = number(invoice.balanceDue);
    const bucket: "paid" | "overdue" | "dueSoon" | "sent" = invoice.status === "PAID" || amount <= 0 ? "paid" : invoice.status === "OVERDUE" || (invoice.dueDate && invoice.dueDate < now) ? "overdue" : invoice.dueDate && invoice.dueDate <= dueSoonCutoff ? "dueSoon" : "sent";
    result[bucket].count += 1; result[bucket].amount += bucket === "paid" ? number(invoice.amountPaid) : amount;
    return result;
  }, { paid: { count: 0, amount: 0 }, sent: { count: 0, amount: 0 }, dueSoon: { count: 0, amount: 0 }, overdue: { count: 0, amount: 0 } });
  const expenseBreakdown = Object.entries(directExpenses.filter((expense) => expense.expenseDate.getFullYear() === year).reduce<Record<string, number>>((result, expense) => { result[expense.category] = (result[expense.category] ?? 0) + number(expense.amount); return result; }, {})).map(([category, amount]) => ({ category, amount })).sort((a, b) => b.amount - a.amount);
