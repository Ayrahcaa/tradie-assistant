...
import { createQuote, type CreateQuoteInput } from "../api/quotes";
...
const initialItem: QuoteItemForm = {
  description: "",
  quantity: "1",
  unitPrice: "",
};
...
export function NewQuoteForm({ onSuccess, onCancel, projectId, projectName, customerId, customerName }: NewQuoteFormProps) {
  ...
  function validateForm(): string | null {
    ...
    if (items.length === 0) {
      return "Add at least one quote item.";
    }
    ...
    <form onSubmit={handleSubmit}>
      <div className="space-y-6 p-6">
          ...
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Add labour, materials and other charges.
              </p>
            </div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={handleChange}
            rows={3}
            placeholder="30% deposit required before commencement."
            className="mt-2 w-full resize-none rounded-xl border border-slate-200 px-3.5 py-3 text-sm outline-none focus:border-slate-500 focus:ring-2 focus:ring-slate-200"
          />
        </div>
      </div>
      ...
      <div className="flex flex-col-reverse gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 sm:flex-row sm:justify-end">
        ...
        >
          ...
          {mutation.isPending ? "Creating quote..." : "Create quote"}
        </button>
      </div>
    </form>
  );
